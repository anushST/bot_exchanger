...
class CoinRedisDataClient:
    ...
    async def get_coin_info(self, coin_name: str, network: str) -> Coin | None:
        ...
        for ex in exchangers:
            ...
            if coin_info:
                code = coin_info.code if coin_info.code else code
                coin = coin_info.coin if coin_info.coin else coin
                net = coin_info.network if coin_info.network else net
                receive = coin_info.receive if coin_info.receive else receive
                send = coin_info.send if coin_info.send else send
                tag_name = (
                    coin_info.tag_name if coin_info.tag_name else tag_name)
        return Coin(
            code=code,
            coin=coin,
            network=net,
