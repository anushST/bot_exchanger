...
class LoadFFIODataToRedis:
    ...
    async def load_currencies_and_networks(self) -> None:
        ...
        await redis_client.delete(self.COINS_KEY)
        await redis_client.sadd(self.COINS_KEY, *coins_set)
    ...
    async def load_fixed_rates(self):
        fixed_rates = await ffio_client.get_fixed_rates()
        await self._load_rates(fixed_rates, 'fixed')

    async def load_float_rates(self):
        float_rates = await ffio_client.get_float_rates()
