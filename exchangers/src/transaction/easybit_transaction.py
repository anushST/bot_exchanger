...
class EasyBitTransaction:
    ...
    async def process(self) -> None:
        ...
        while expired_retries < 60 * 24:
            ...
            if transaction.status == TransactionStatuses.NEW.value:
                logger.error(f'Invalid transaction status NEW for {self.transaction_id}')
                break
    ...
    async def _handle_new(self, transaction: Transaction) -> None:
        try:
            ...
            # Формирование запроса для создания ордера
            ...
            # Вызов API EasyBit для создания ордера
    ...
    async def _handle_handled(self, transaction: Transaction) -> None:
        try:
            ...
            # Маппинг статусов EasyBit на статусы транзакций
            status_mapping = {
            ...
            if new_status is None:
                logger.error(f'Unknown status retrieved for transaction {self.transaction_id}: {response.data.status}')
